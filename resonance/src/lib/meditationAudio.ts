 ...
 * recording covers all of them. Served from a public Supabase Storage
 * bucket rather than bundled with the app, so a device only ever downloads
 * the handful of specific clips it actually plays — not all 15 languages.
 */
const STORAGE_BASE = `${SUPABASE_URL}/storage/v1/object/public/meditation-audio`
...
export function meditationLineAudioUrl(locale: Locale, line: MessageKey): string | null {
  ...
  return `${STORAGE_BASE}/meditations/${locale}/${line}.mp3`
}
...
export function ambientTrackUrl(minutes: number): string {
  ...
  return `${STORAGE_BASE}/ambient/${closest}min.mp3`
}
