...
/**
 * The store computes today's transit on load; this just catches the case
 * where the app was left open across midnight and re-rolls the reading.
 */
...
type DashView = 'home' | 'chart' | 'horoscope' | 'journal' | 'mood'
...
function App() {
  ...
  return (
    <>
      <AudioBridge />
      <Layout active={tab} onTabChange={goTab}>
        ...
        {tab === 'frequencies' && <FrequenciesView />}
        {tab === 'breathwork' && <BreathworkView onStartRitual={() => setRitual(true)} />}
        {tab === 'apothecary' && <ApothecaryView />}
      </Layout>
    </>
  )
}
        ...
        {SOLFEGGIO_PRESETS.map((preset) => {
          ...
          return (
            <li key={preset.frequency}>
              <button
                type="button"
                onClick={() => tuneTo(preset.frequency)}
